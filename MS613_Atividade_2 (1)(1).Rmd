...
# coloque os comandos para os cálculos
# 2.752 da tabela
c(b1_hat -2.306*erro_b1,b1_hat + 2.306*erro_b1)
```
...
```{r}
c(-2.576,2.576)*erro_b1_height+b1_hat_height

lm(wt~ht, data = Htwt)
confint(lm(dheight~mheight, data = Heights))
```
...
```{r}
# coloque os comandos para os cálculos
c(-2.576,2.576)*sqrt(s_2_height*(1+1/1375+((64-x_bar_height)^2)/s_xx_height))+64*b1_hat_height+b0_hat_height
```
...
# coloque os comandos
set.seed(204244)
treino <- Heights[sample(c(1:length(Heights[,1])),size = round(2*length(Heights[,1])/3),replace = FALSE),]
validação <- Heights[-sample(c(1:length(Heights[,1])),size = round(2*length(Heights[,1])/3),replace = FALSE),]
```
